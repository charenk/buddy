  ...
  try {
    ...
    if (processedCount > 0) {
      figma.ui.postMessage({
        ...
        message: `✅ Processed ${processedCount} @buddy comment(s)! Check the replies.`
      });
    } else {
      figma.ui.postMessage({
        type: 'no-comments-found',
        message: 'ℹ️ No @buddy comments found. Add a comment with "@buddy" to any frame first.'
      });
    }
...
async function processBuddyComment(comment) {
  try {
    ...
    console.log('✅ AI response added to comment');
...
function generateAIAnalysis(nodeInfo, question) {
  ...
  analysis += `📏 **Dimensions:** ${Math.round(width)} × ${Math.round(height)}px\n`;
  
  const aspectRatio = width / height;
  if (aspectRatio > 2) {
    analysis += "⚠️ Very wide element - consider mobile responsiveness\n";
  } else if (aspectRatio < 0.5) {
    analysis += "⚠️ Very tall element - consider horizontal layout\n";
  } else {
    analysis += "✅ Good proportions for most screen sizes\n";
  }
  
  // Layout analysis for frames
  if (type === 'FRAME' && layoutMode) {
    analysis += `\n📐 **Layout:** ${layoutMode === 'HORIZONTAL' ? 'Horizontal' : 'Vertical'} layout\n`;
    
    if (children && children.length > 0) {
      analysis += `📦 **Children:** ${children.length} elements\n`;
      
      // Analyze child spacing
      const visibleChildren = children.filter(child => child.visible);
      if (visibleChildren.length > 1) {
        analysis += "✅ Multiple elements with good organization\n";
      }
    }
  }
  ...
  if (cornerRadius && cornerRadius > 0) {
    if (cornerRadius > 20) {
      analysis += `🎨 **Corners:** Large radius (${cornerRadius}px) - soft, friendly feel\n`;
    } else {
      analysis += `🎨 **Corners:** Subtle radius (${cornerRadius}px) - modern look\n`;
    }
  } else {
    analysis += `💡 **Corners:** Consider adding corner radius for modern appearance\n`;
  }
