 ...
 */
export async function getUserByFigmaId(figmaUserId) {
  try {
    const { data, error } = await supabase
      .from('users')
      .select('*')
 ...
 */
export async function trackUsage(userId, fileKey, interactionType, apiKeyUsed, responseTimeMs, tokensUsed) {
  try {
    const { data, error } = await supabase
      .rpc('track_usage', {
        user_uuid: userId,
 ...
 */
export async function incrementTrialCount(userId) {
  try {
